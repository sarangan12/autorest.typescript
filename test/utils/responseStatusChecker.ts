...
export const responseStatusChecker202 = {
  onResponse: (response: any) => {
    assert.equal(response.status, 202, "Unexpected status code.");
  }
};

export const responseStatusChecker204 = {
  onResponse: (response: any) => {
    assert.equal(response.status, 204, "Unexpected status code.");
  }
};

export const responseStatusChecker301 = {
  onResponse: (response: any) => {
    assert.equal(response.status, 301, "Unexpected status code.");
  }
};

export const responseStatusChecker302 = {
  onResponse: (response: any) => {
    assert.equal(response.status, 302, "Unexpected status code.");
  }
};

export const responseStatusChecker404 = {
  onResponse: (response: any) => {
    assert.equal(response.status, 404, "Unexpected status code.");
  }
};
